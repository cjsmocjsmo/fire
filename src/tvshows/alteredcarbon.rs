...
fn altered_carbon_season_1() -> Result<Vec<TvShowsStruc>> {
    ...
    let mut stmt1  = db.prepare("
        ...
        WHERE name='Altered Carbon', season='01'
    ")?;
    ...
    for mo in altered_carbon_tv_iter {
        let m = mo.unwrap();
        tv_vec.push(m);
    }
...
fn altered_carbon_season_2() -> Result<Vec<TvShowsStruc>> {
    ...
    let mut stmt2  = db.prepare("
        SELECT id, fireid, idx, category, name, season, episode, size, httppath, vidtype FROM tvshows
        WHERE name='Altered Carbon', season='02'
    ")?;
...
pub async fn altered_carbon_tv(season: String) -> Result<Vec<Vec<TvShowsStruc>>> {
    let mut epilist = Vec::new();

    if season == "1" {
        let zlist = altered_carbon_season_1();
        if let Ok(z) = zlist {
            epilist.push(z);
        }
    };
    ...
    if season == "2" {
        let wlist = altered_carbon_season_2();
        if let Ok(wl) = wlist {
            epilist.push(wl);
        }
    };
