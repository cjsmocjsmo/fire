...
fn night_sky_season_1() -> Result<Vec<TvShowsStruc>> {
    ...
    let mut stmt1  = db.prepare("
        ...
        WHERE category='Night Sky' AND season='01'
    ")?;
...
pub async fn night_sky_tv(season: String) -> Result<Vec<Vec<TvShowsStruc>>> {
    let mut epilist = Vec::new();

    if season == "01" {
        let zlist = night_sky_season_1();
        if let Ok(z) = zlist {
            epilist.push(z);
        }
    };
