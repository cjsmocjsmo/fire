...
fn rings_of_power_season_1() -> Result<Vec<TvShowsStruc>> {
    ...
    let mut stmt1  = db.prepare("
        ...
        WHERE category='Rings Of Power' AND season='01'
    ")?;
...
// fn rings_of_power_season_2() -> Result<Vec<TvShowsStruc>> {
//     let db = Connection::open("fire.db")?;
//     let mut tv_vec = Vec::new();
//     let mut stmt2  = db.prepare("
//         SELECT id, fireid, idx, category, name, season, episode, size, httppath, vidtype FROM tvshows
//         WHERE category='Rings Of Power' AND season='02'
//     ")?;
//     let rings_of_power_tv_iter = stmt2.query_map([], |row| {
//         Ok(TvShowsStruc {
//             id: row.get(0)?,
//             fireid: row.get(1)?,
//             idx: row.get(2)?,
...
pub async fn rings_of_power_tv(season: String) -> Result<Vec<Vec<TvShowsStruc>>> {
    ...
    // if season == "02" {
    //     let wlist = rings_of_power_season_2();
    //     if let Ok(wl) = wlist {
    //         epilist.push(wl);
    //     }
    // };
